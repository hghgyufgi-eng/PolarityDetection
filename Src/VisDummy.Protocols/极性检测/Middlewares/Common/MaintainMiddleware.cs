...
namespace VisDummy.Protocols.极性检测.Middlewares.Common
{
    public class MaintainMiddleware : IWorkMiddleware<ScanContext>
    {
        ...
        private readonly 极性检测Scanner _scanner;

        public MaintainMiddleware(ILogger<MaintainMiddleware> logger, IOptionsMonitor<S7ScanOpt> scanOptsMonitor, IMediator mediator, 极性检测Scanner scanner)
        {
            _logger = logger;
