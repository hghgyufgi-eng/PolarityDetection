...
namespace VisDummy.Protocols.Loading.Middlewares
{
        ...
        >(flusher, logger, mediator)
    {
        ...
        protected override async Task HandleErrAsync(MstMsg_3DStation pending, StationErrWrap_MK err)
        {
            ...
            //根据errorcode输出信息
            await this.RecordLogAsync(LogLevel.Error, $"{ErrorEnum.GetErrMessage(err.ResultStatus)}");
        }

        protected override async Task HandleOkAsync(MstMsg_3DStation pending, StationOkWrap_MK descriptions)
        {
            pending.SetOn(true, new CmdArg_Reply3D(descriptions.result,descriptions.foam, descriptions.Floor, descriptions.Column, descriptions.Direction, descriptions.PreciseX, descriptions.PreciseY, descriptions.PreciseZ, descriptions.PreciseA, descriptions.PreciseB, descriptions.PreciseC));
