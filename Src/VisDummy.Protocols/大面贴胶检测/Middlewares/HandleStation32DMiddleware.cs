...
{
    public class HandleStation32DMiddleware(大面贴胶检测Flusher flusher, IVisProc visproc, IVisParams visParams, ILogger<HandlePlcRequestMiddlewareBase<DevMsg_2DStation, MstMsg_2DStation, StationArgs_Large, StationOkWrap_大面贴胶检测, StationErrWrap_大面贴胶检测>> logger, IMediator mediator) : HandlePlcRequestMiddlewareBase<
        DevMsg_2DStation, MstMsg_2DStation, StationArgs_Large,
        StationOkWrap_大面贴胶检测, StationErrWrap_大面贴胶检测
        >(flusher, logger, mediator)
    {
        public override string PlcName => PlcNames.PLCNAME_大面贴胶检测 + "NG替换";

        public override string ProcName => Language.Msg_正常工作拍照 + "3";
        ...
        public override DevMsg_2DStation RefIncoming(ScanContext ctx) => ctx.DevMsg.Station2D3_NGReplace;


        public override MstMsg_2DStation RefPending(ScanContext ctx) => ctx.MstMsg.Station2D3_NGReplace;
        ...
        protected override async Task<FSharpResult<StationOkWrap_大面贴胶检测, StationErrWrap_大面贴胶检测>> HandleArgsAsync(StationArgs_Large args)
        {
            ...
            var res = from r1 in visParams.SetTriggerGlobalParams()
                      ...
                      from r2 in visproc.大面贴胶检测NG替换ProcAsync(args)
                      select r2;
            return await res;
        }
