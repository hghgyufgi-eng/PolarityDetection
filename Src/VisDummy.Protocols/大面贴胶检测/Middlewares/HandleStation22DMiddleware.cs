...
{
    public class HandleStation22DMiddleware(大面贴胶检测Flusher flusher, IVisProc visproc, IVisParams visParams, ILogger<HandlePlcRequestMiddlewareBase<DevMsg_2DStation, MstMsg_2DStation, StationArgs_Large, StationOkWrap_大面贴胶检测, StationErrWrap_大面贴胶检测>> logger, IMediator mediator) : HandlePlcRequestMiddlewareBase<
        DevMsg_2DStation, MstMsg_2DStation, StationArgs_Large,
        StationOkWrap_大面贴胶检测, StationErrWrap_大面贴胶检测
        >(flusher, logger, mediator)
    {
        public override string PlcName => PlcNames.PLCNAME_大面贴胶检测 + "外轨道";
        ...
        public override DevMsg_2DStation RefIncoming(ScanContext ctx) => ctx.DevMsg.Station2D2;


        public override MstMsg_2DStation RefPending(ScanContext ctx) => ctx.MstMsg.Station2D2;
        ...
        protected override async Task<FSharpResult<StationOkWrap_大面贴胶检测, StationErrWrap_大面贴胶检测>> HandleArgsAsync(StationArgs_Large args)
        {
            ...
            var res = from r1 in visParams.SetTriggerGlobalParams()
                      ...
                      from r2 in visproc.大面贴胶检测2ProcAsync(args)
                      select r2;
            return await res;
        }
