...
namespace VisDummy.Protocols.大面贴胶检测.Middlewares.Common
{
    public class MaintainMiddleware : IWorkMiddleware<ScanContext>
    {
        ...
        private readonly 大面贴胶检测Scanner _scanner;

        public MaintainMiddleware(ILogger<MaintainMiddleware> logger, IOptionsMonitor<S7ScanOpt> scanOptsMonitor, IMediator mediator, 大面贴胶检测Scanner scanner)
        {
            _logger = logger;
