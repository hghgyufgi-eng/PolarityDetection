...
namespace VisDummy.MKVMs.Common
{
    ...
    public static class Vision3DExtension
    {
        ...
        public static FSharpResult<Vision3D_110_DTO, string> Analyze110(this string cmd)
        {
            ...
            if (cmds.Length <= 14)
            {
                return $"Failed To Parse 110：数组长度={cmds.Length},不足15".ToErrResult<Vision3D_110_DTO, string>();
            }
            if (cmds[0] != "110")
            {
            ...
            var r11 = ushort.TryParse(cmds[12], out var high);
            if (!r11)
                return $"Failed To Parse 110：电芯列数非ushort类型".ToErrResult<Vision3D_110_DTO, string>();
            var r9 = ushort.TryParse(cmds[13], out var postion);
            if (!r9)
                return $"Failed To Parse 110：电芯来料方向非ushort类型".ToErrResult<Vision3D_110_DTO, string>();

            var r10 = ushort.TryParse(cmds[14], out var floor);
            if (!r10)
                return $"Failed To Parse 110：泡棉层高非ushort类型".ToErrResult<Vision3D_110_DTO, string>();

            return new Vision3D_110_DTO { ProcessStatus = cmds[1], X = x, Y = y, Z = z, A = a, B = b, C = c, ResultStatus = status, Column = col, Postion = postion, Floor = floor,High=high }.ToOkResult<Vision3D_110_DTO, string>();
        }
    }
}
